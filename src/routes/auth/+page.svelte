...
<div class="flex h-full w-full flex-col items-center justify-center bg-primary/10 p-6">
	<div class="card w-full max-w-sm bg-base-100">
		...
		>
			...
			<button type="button" class="btn flex w-full gap-2 btn-outline">
				<AppleIcon />
				{m.routes_auth_oauth_button_apple()}
			</button>

			<button type="button" class="btn flex w-full gap-3 btn-outline">
				<GoogleIcon />
			...
			{#if !token}
				<fieldset class="fieldset">
					<label class="fieldset-label" for="email">{m.routes_auth_input_email_label()}</label>
					<input
						type="email"
			...
			{:else}
				<fieldset class="fieldset">
					<label class="fieldset-label" for="secret">{m.routes_auth_input_secret_label()}</label>
					<input
						type="text"
						placeholder="123456"
						id="secret"
